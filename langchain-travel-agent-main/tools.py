...
from config import AMAP_MCP_URL, BING_MCP_URL, TRAIN12306_MCP_URL, Config
...
_TOOL_RESULT_CACHE: dict[str, object] = {}
...
def _cache_key(tool_name: str, payload) -> str:
    """构造稳定缓存键，参数无法 JSON 化时退回字符串。"""
    try:
...
def _cached_tool_call(tool_name: str, payload, runner):
    """工具级内存缓存，减少同参数 MCP 重复调用。"""
    key = _cache_key(tool_name, payload)
    if key in _TOOL_RESULT_CACHE:
        logger.info("[ToolCache] 命中缓存：%s", tool_name)
        return _TOOL_RESULT_CACHE[key]

    result = runner()
    if len(_TOOL_RESULT_CACHE) >= Config.TOOL_CACHE_MAX_SIZE:
        oldest_key = next(iter(_TOOL_RESULT_CACHE))
    ...
    if not coroutine:
        if hasattr(t, "ainvoke"):
            if args_schema is not None:
                ...
                def _sync(**kwargs):
                    return _cached_tool_call(
                        name,
                        kwargs,
                        lambda: _run_coroutine_sync(t.ainvoke, kwargs),
                    )
                ...
                def _sync(tool_input):
                    return _cached_tool_call(
                        name,
                        tool_input,
                        lambda: _run_coroutine_sync(t.ainvoke, tool_input),
                    )
    ...
    if args_schema is not None:
        ...
        def _sync(**kwargs):
            return _cached_tool_call(
                name,
                kwargs,
                lambda: _run_coroutine_sync(coroutine, **kwargs),
            )
        ...
        def _sync(tool_input):
            return _cached_tool_call(
                name,
                tool_input,
                lambda: _run_coroutine_sync(coroutine, tool_input),
            )
...
def _get_server_configs() -> dict:
    ...
    if BING_MCP_URL:
        ...
        logger.info("[MCP] 已添加 Bing 中文搜索服务")

    if TRAIN12306_MCP_URL:
        servers["train-12306"] = {
            "transport": "streamable_http",
            "url": TRAIN12306_MCP_URL,
        }
        logger.info("[MCP] 已添加 12306 火车票服务")
...
def init_mcp_client() -> MultiServerMCPClient:
    ...
    return MultiServerMCPClient(_get_server_configs())
...
def load_tools() -> List:
    ...
    for server_name, server_config in server_configs.items():
        try:
            ...
            tools = asyncio.run(asyncio.wait_for(single_client.get_tools(), timeout=60))
            raw_tools.extend(tools)
            logger.info("[MCP] %s 加载成功，%d 个工具", server_name, len(tools))
        except Exception as e:
            logger.warning("[MCP] %s 加载失败（跳过）: %s", server_name, e)
