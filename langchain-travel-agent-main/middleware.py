...
def pin_cities_and_adcodes(state: AgentState, runtime) -> dict | None:
    ...
    sys_msg = SystemMessage(
        content=(
            ...
            "如需查询火车票、车次、车站、换乘方案，使用 MCP(train-12306) 的工具："
            "query_tickets / search_stations / query_transfer / get_train_route_stations。"
            + (
                "如需搜索实时网络信息、攻略、新闻、网页资料，使用 MCP(bing-search) 的工具："
                "bing_search / crawl_webpage。"
                if any(name in get_tool_names_sorted() for name in ("bing_search", "crawl_webpage"))
                else ""
            )
            +
            "风格：\"轻松不赶\"；"
            "输出包含可验证的检索要点（如POI名、路线段、adcode等）。"
            "\n【并行调用策略】强烈建议在同一轮中并行调用多个工具以提升效率："
            "\n- 示例：同时调用 searchPOI（获取景点）+ getRoute（规划路线）+ getWeather（查询天气）"
            "\n- 优势：一次性收集完整信息，减少往返轮次"
            "\n【停止条件-必须遵守】满足以下任一条件时，必须立即停止调用工具，直接给出最终答案："
            "\n1. 已获取关键信息：目标城市、2-3个POI、基本路线信息"
            f"\n2. 工具调用达到{Config.MAX_TOOL_ROUNDS}回合上限（严格限制，因启用并行调用）"
            "\n3. 收到\"强制停止\"提示"
            "\n【执行要求】达到停止条件后，必须直接输出文本答案，禁止继续调用工具。若信息不足，说明\"仍需补充的信息\"。"
            + city_tips
            + debug_info
        )
    )
